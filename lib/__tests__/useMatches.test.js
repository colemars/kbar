...
function WithLongNamesComponent() {
    var action1 = (0, utils_1.createAction)({
        name: "Action: This is a long name ending by toto",
    });
    var action2 = (0, utils_1.createAction)({
        name: "Action: This is a long name also ending by toto",
    });
    var action3 = (0, utils_1.createAction)({
        name: "Action: This is a long name ending by titi",
    });
    return (React.createElement(KBarContextProvider_1.KBarProvider, { actions: [action1, action2, action3] },
        React.createElement(Search, null),
        React.createElement(Results, null)));
}
function WithIgnoredRootSearchComponent() {
    var visibleAction = (0, utils_1.createAction)({
        name: "Visible Action",
    });
    var hiddenAtRootAction = (0, utils_1.createAction)({
        name: "Hidden Root Action",
        keywords: "visible query",
        ignoreInRootSearch: true,
    });
    var parentAction = (0, utils_1.createAction)({
        name: "Parent Action",
    });
    var nestedHiddenAction = (0, utils_1.createAction)({
        name: "Nested Hidden Action",
        parent: parentAction.id,
        keywords: "visible query",
        ignoreInRootSearch: true,
    });
    return (React.createElement(KBarContextProvider_1.KBarProvider, { actions: [
            visibleAction,
            hiddenAtRootAction,
            parentAction,
            nestedHiddenAction,
...
describe("useMatches", function () {
    ...
    describe("With long names", function () {
        var utils;
        beforeEach(function () {
            utils = setup(WithLongNamesComponent);
        });
        it("returns result matching the query even if match is on a word far in the name", function () {
            var input = utils.input;
            react_1.fireEvent.change(input, { target: { value: "toto" } });
            var results = utils.getAllByText(/Action/i);
            expect(results.length).toEqual(2);
            expect(results[0].textContent).toEqual("Action: This is a long name ending by toto");
            expect(results[1].textContent).toEqual("Action: This is a long name also ending by toto");
        });
    });
    describe("With root-only hidden actions", function () {
        var utils;
        beforeEach(function () {
            utils = setup(WithIgnoredRootSearchComponent);
        });
        it("keeps ignored actions out of root search results", function () {
            var input = utils.input;
            react_1.fireEvent.change(input, { target: { value: "visible query" } });
